...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws ServletException, IOException {
        ...
        if (session == null || session.getAttribute("username") == null) {
            if (requestedUri.contains("addfood") || requestedUri.contains("foodlist") || requestedUri.contains("meal") || requestedUri.contains("logout")) {
                ((HttpServletResponse) resp).sendRedirect("login.htm");
            } else {
            ...
            if (requestedUri.contains("login")) {
                ((HttpServletResponse) resp).sendRedirect("assign.htm");
            } else {
                chain.doFilter(req, resp);
            }
        } else {
